"""
    python src/run_camera_resnet_detect.py --model-path models/resnet_resnet50_detect_best.pt --source webcam
    python src/run_camera_resnet_detect.py --model-path models/resnet_resnet50_detect_best.pt --source droidcam \
...
logger = logging.getLogger("camera_resnet_detect")
...
def get_device():
    # No MPS branch: this model's ROIAlign/NMS ops are numerically unstable
    # on Apple's MPS backend with this torch/torchvision version (see
    # train_resnet_detect.py's get_device for how this was confirmed) - a
    # model that predicts NaN boxes will silently draw nothing at inference
    # time too, so this stays CPU-only for the same reason.
...
def main():
    ...
    parser.add_argument("--model-path", default="models/resnet_resnet50_detect_best.pt")
