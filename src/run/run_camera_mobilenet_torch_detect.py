"""
    python src/run/run_camera_mobilenet_torch_detect.py --model-path models/mobilenet_torch_v2_detect_best.pt --source webcam
...
logger = logging.getLogger("camera_mobilenet_torch_detect")
...
class MobileNetFPNBackbone(nn.Module):
    """Must match train_mobilenet_torch_detect.py's MobileNetFPNBackbone exactly - same
    features.6/.12/.16 -> p3/p4/p5 feature extraction feeding a plain 3-level FPN."""

    def __init__(self, mobilenet, height, width, out_channels=256):
        super().__init__()
        self.body = create_feature_extractor(
            mobilenet, return_nodes={"features.6": "p3", "features.12": "p4", "features.16": "p5"},
        )
        with torch.no_grad():
...
def build_model(num_classes, height, width):
    backbone = MobileNetFPNBackbone(mobilenet_v3_large(weights=None), height, width)
...
def main():
    ...
    parser.add_argument("--model-path", default="models/mobilenet_torch_v2_detect_best.pt")
    ...
    window = "Box Detector - MobileNetV3 RetinaNet Torch (q to quit)"
    last_log_time = 0.0
