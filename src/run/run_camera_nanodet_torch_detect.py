"""
    python src/run/run_camera_nanodet_torch_detect.py --model-path models/nanodet_torch_best.pt --source webcam
    python src/run/run_camera_nanodet_torch_detect.py --model-path models/nanodet_torch_best.pt \
        --camera-index 1 --save-dir raw_capture --save-interval 2

PyTorch counterpart of run_camera_nanodet_tf_detect.py, for checkpoints produced by
train_nanodet_torch_detect.py (NanoDet-Plus: ShuffleNetV2 backbone, PAN neck, DFL head -
see that file's NanoDetModel/decode_single for the architecture and decode math, which this
mirrors exactly in eval mode).
"""
import argparse
import logging
import os
...
logger = logging.getLogger("camera_nanodet_torch_detect")

IMAGENET_MEAN = [0.485, 0.456, 0.406]
IMAGENET_STD = [0.229, 0.224, 0.225]

# Must match train_nanodet_torch_detect.py's copies of these exactly
STRIDES = (8, 16, 32)
...
class DepthwiseSeparableConv(nn.Module):
    """Must match train_nanodet_torch_detect.py's DepthwiseSeparableConv exactly."""
...
class Scale(nn.Module):
    """Must match train_nanodet_torch_detect.py's Scale exactly."""
...
class NanoDetNeck(nn.Module):
    """Must match train_nanodet_torch_detect.py's NanoDetNeck exactly."""
...
class NanoDetHead(nn.Module):
    """Must match train_nanodet_torch_detect.py's NanoDetHead exactly."""
...
class NanoDetModel(nn.Module):
    """Must match train_nanodet_torch_detect.py's NanoDetModel exactly."""

    def __init__(self, num_classes, feat_channels=FEAT_CHANNELS, stacked_convs=STACKED_CONVS, reg_max=7):
        super().__init__()
...
def decode_single(cls_scores, bbox_preds, strides, img_size, reg_max, score_thresh,
                   nms_iou=NMS_IOU_THRESH, max_dets=MAX_DETECTIONS):
    """Must match train_nanodet_torch_detect.py's decode_single exactly (score_thresh here
    is the camera script's --conf instead of that file's fixed EVAL_SCORE_THRESH)."""
...
def load_model(model_path, device):
    ...
    reg_max = checkpoint["reg_max"]

    model = NanoDetModel(len(class_names), reg_max=reg_max)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.to(device).eval()
    return model, class_names, img_size, reg_max
...
@torch.no_grad()
def detect(model, class_names, img_size, reg_max, frame, device, conf_threshold):
    height, width = frame.shape[:2]
    resized = cv2.resize(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), (img_size, img_size), interpolation=cv2.INTER_LINEAR)
    img_tensor = normalize(to_tensor(resized), mean=IMAGENET_MEAN, std=IMAGENET_STD).unsqueeze(0).to(device)

    cls_scores, bbox_preds = model(img_tensor)
    boxes, scores, labels = decode_single(
        [c[0] for c in cls_scores], [b[0] for b in bbox_preds], STRIDES, img_size, reg_max, conf_threshold,
    )
...
def main():
    ...
    parser.add_argument("--model-path", default="models/nanodet_torch_best.pt")
    ...
    model, class_names, img_size, reg_max = load_model(args.model_path, device)
    logger.info(f"Loaded model, classes: {class_names}, img_size: {img_size}, confidence threshold: {args.conf:.0%}")

    cap = open_capture(args)
    window = "Box Detector - NanoDet Torch (q to quit)"
    last_log_time = 0.0
    last_save_time = 0.0
    ...
    try:
        while True:
            ...
            detections = detect(model, class_names, img_size, reg_max, frame, device, args.conf)
