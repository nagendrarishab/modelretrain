"""
    python src/run/run_camera_resnet_fasterrcnn_detect.py --model-path models/resnet_fasterrcnn_detect_best.pt --source webcam
    python src/run/run_camera_resnet_fasterrcnn_detect.py --model-path models/resnet_fasterrcnn_detect_best.pt --source droidcam \
...
logger = logging.getLogger("camera_resnet_fasterrcnn_detect")
...
def build_model(num_classes, variant):
    ctor = fasterrcnn_resnet50_fpn_v2 if variant == "resnet50_fpn_v2" else fasterrcnn_resnet50_fpn
    # weights_backbone=None here too - real weights come from the
    # checkpoint's state_dict a moment later, not from a fresh download.
    return ctor(weights=None, weights_backbone=None, num_classes=num_classes)
...
def load_model(model_path, device):
    ...
    model = build_model(len(class_names) + 1, checkpoint["variant"])
...
def main():
    ...
    parser.add_argument("--model-path", default="models/resnet_fasterrcnn_detect_best.pt")
    ...
    window = "Box Detector - ResNet Faster R-CNN (q to quit)"
    last_log_time = 0.0
