"""
    python src/run/run_camera_mobilenet_tf_detect.py --model-path models/mobilenet_tf_mobilenetv3_large_detect_best.keras --source webcam
    python src/run/run_camera_mobilenet_tf_detect.py --model-path models/mobilenet_tf_mobilenetv3_large_detect_best.keras --source droidcam \
...
is otherwise byte-identical code, just pointed at a MobileNetV3-Large
checkpoint.

The one real difference: loading a saved checkpoint requires the custom
`PyramidMobileNetBackbone` class (train_mobilenet_tf_detect.py) to be
registered in this process first - `keras.saving.load_model()` fails with
`Could not locate class 'PyramidMobileNetBackbone'` otherwise, since the
`@register_keras_serializable` decorator only takes effect once the class
has actually been imported. Same reason run_camera_nanodet_tf_detect.py
carries its own copies of its custom layer classes rather than importing
the trainer - this script defines an identical copy below so it works
standalone, without importing train_mobilenet_tf_detect.py.
"""
import argparse
import logging
import os
...
logger = logging.getLogger("camera_mobilenet_tf_detect")

# Must match train_mobilenet_tf_detect.py's copy exactly.
...
def main():
    ...
    parser.add_argument("--model-path", default="models/mobilenet_tf_mobilenetv3_large_detect_best.keras")
    ...
    window = "Box Detector - MobileNetV3 RetinaNet TF (q to quit)"
    last_log_time = 0.0
