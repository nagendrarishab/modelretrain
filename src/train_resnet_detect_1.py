...
def get_device():
    # Deliberately no MPS branch here (unlike the YOLO scripts): Faster R-CNN's
    # ROIAlign/NMS ops are unstable on Apple's MPS backend with this
    # torch/torchvision version - it doesn't just NaN, it reliably crashes the
    # whole machine. CPU is the only safe option here on a Mac.
...
def offload_to_device(module, device):
    """Runs `module` on `device`, moving its input there and its output back
    to CPU - without wrapping it in a new nn.Module, so state_dict keys are
    unchanged. Only safe for the plain conv/batchnorm/relu ResNet+FPN
    backbone: RPN/ROIAlign/NMS must stay on CPU since those ops are unstable
    on Apple's MPS backend with this torch/torchvision version and have
    previously crashed the machine."""
    module.to(device)
    original_forward = module.forward

    def forward(x):
        out = original_forward(x.to(device))
        return {k: v.to("cpu") for k, v in out.items()}

    module.forward = forward
    return module


def build_model(backbone_name, num_classes, backbone_device=None, min_size=800, max_size=1333):
    ...
    return FasterRCNN(backbone, num_classes=num_classes, min_size=min_size, max_size=max_size)
...
def main():
    ...
    parser.add_argument("--min-size", type=int, default=320,
                         help="Shorter image side after resize (torchvision default is 800; "
                              "this dataset's boxes cover most of the frame so a smaller value "
                              "trains much faster with no meaningful accuracy loss)")
    parser.add_argument("--max-size", type=int, default=569,
                         help="Longer image side cap after resize (torchvision default is 1333)")
    ...
    model = build_model(args.backbone, num_classes, backbone_device=backbone_device,
    ...
    model = build_model(best["backbone"], len(best["class_names"]) + 1, backbone_device=backbone_device,
                         min_size=args.min_size, max_size=args.max_size)
    if backbone_device is None:
        model = model.to(device)
