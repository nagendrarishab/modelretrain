...
TensorFlow/KerasHub RetinaNet detector on a DenseNet backbone - the same
pipeline as train_resnet_tf_detect.py/train_efficientnet_tf_detect.py, just
swapping the backbone preset. DenseNet's keras_hub.models.DenseNetBackbone
already exposes the same pyramid_outputs interface ResNet's/EfficientNet's
do (confirmed empirically: RetinaNetBackbone(image_encoder=...) builds and
runs a forward pass unchanged), so no custom pyramid-tapping code is needed
here, unlike train_mobilenet_tf_detect.py.
...
Adam@5e-5, fixed-size image loading, 0-indexed classes with no reserved
background label) - the only differences are the backbone preset and the
--backbone flag to pick a DenseNet depth.
"""
import argparse
...
def main():
    ...
    parser.add_argument("--backbone", default="densenet_121_imagenet", choices=BACKBONE_CHOICES,
                         help="DenseNet depth/preset - 121 (fastest), 169, or 201 (most accurate, slowest)")
    ...
    parser.add_argument("--height", type=int, default=480,
                         help="Fixed input height to train at - keep this and --width divisible by 32 "
                              ...
                              "in RetinaNet's FeaturePyramid can mismatch shapes between levels")
    parser.add_argument("--width", type=int, default=640, help="Fixed input width to train at - see --height")
    parser.add_argument("--output", default=None,
                         help="Output path. Defaults to models/densenet_tf_<backbone-depth>_detect_best.keras")
    args = parser.parse_args()

    if args.output is None:
        depth = args.backbone.split("_")[1]  # "densenet_121_imagenet" -> "121"
        args.output = f"models/densenet_tf_densenet{depth}_detect_best.keras"
