"""
    python src/train/train_resnet_torch_detect.py --backbone resnet18 --epochs 30
    ...
    ./.venv/bin/python src/train/train_resnet_torch_detect.py --epochs 2 --batch-size 16

    # CPU run - pair with a GPU run of another backbone for real parallelism (see --device):
    ./.venv/bin/python src/train/train_resnet_torch_detect.py --epochs 2 --batch-size 4 --device cpu
"""
import argparse
import os
...
BACKBONE_BUILDERS = {
    "resnet18": (resnet18, ResNet18_Weights.IMAGENET1K_V1),   # matches the TF version's hardcoded resnet_18_imagenet
...
def make_emitter(log_dir):
    ...
    log_path = Path(log_dir) / f"train_resnet_torch_{datetime.now():%Y%m%d_%H%M%S}.log"
...
class ResNetFPNBackbone(nn.Module):
    """ResNet body + a plain 3-level FPN (P3/P4/P5 only, out_channels=256). Mirrors
...
def build_model(num_classes, height, width, backbone_name):
    ...
    backbone = ResNetFPNBackbone(ctor(weights=weights), height, width)
    ...
    # for its 5-level default, just trimmed to the 3 levels ResNetFPNBackbone produces
...
def main():
    ...
    parser.add_argument("--backbone", default="resnet18", choices=list(BACKBONE_BUILDERS),
                         help="ResNet depth - 18 (matches the TF version, fastest), 34, or 50 "
    ...
    parser.add_argument("--output", default=None,
                         help="Output path. Defaults to models/resnet_torch_<backbone>_detect_best.pt")
    ...
    if args.output is None:
        args.output = f"models/resnet_torch_{args.backbone}_detect_best.pt"
    ...
    emit(
        "\nNote: src/evaluate_models.py doesn't recognize the 'resnet-retinanet-torch' "
