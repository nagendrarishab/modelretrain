"""
    python src/train/train_mobilenet_torch_detect.py --weights v2 --epochs 30
    ...
    ./.venv/bin/python src/train/train_mobilenet_torch_detect.py --epochs 2 --batch-size 16

    # CPU run - pair with a GPU run of another backbone for real parallelism (see --device):
    ./.venv/bin/python src/train/train_mobilenet_torch_detect.py --epochs 2 --batch-size 4 --device cpu
"""
import argparse
import os
...
# TF version offers imagenet vs imagenet_21k pretrain; torchvision's closest equivalent is
# two weight recipes for the same architecture (V2 is a later, generally stronger recipe)
WEIGHTS_CHOICES = {
    "v1": MobileNet_V3_Large_Weights.IMAGENET1K_V1,
    "v2": MobileNet_V3_Large_Weights.IMAGENET1K_V2,
}


def make_emitter(log_dir):
    Path(log_dir).mkdir(parents=True, exist_ok=True)
    log_path = Path(log_dir) / f"train_mobilenet_torch_{datetime.now():%Y%m%d_%H%M%S}.log"
...
class MobileNetFPNBackbone(nn.Module):
    ...
    def __init__(self, mobilenet, height, width, out_channels=256):
        ...
        self.body = create_feature_extractor(
            mobilenet, return_nodes={
                "features.6": "p3",   # stride 8
                "features.12": "p4",  # stride 16
                "features.16": "p5",  # stride 32
            },
        )
        with torch.no_grad():
...
def build_model(num_classes, height, width, weights_key):
    backbone = MobileNetFPNBackbone(mobilenet_v3_large(weights=WEIGHTS_CHOICES[weights_key]), height, width)
    ...
    # for its 5-level default, just trimmed to the 3 levels MobileNetFPNBackbone produces
...
def main():
    ...
    parser.add_argument("--weights", default="v2", choices=list(WEIGHTS_CHOICES),
                         help="MobileNetV3-Large pretrain recipe - v2 (default, generally stronger) or v1")
    ...
    parser.add_argument("--output", default=None,
                         help="Output path. Defaults to models/mobilenet_torch_<weights>_detect_best.pt")
    ...
    if args.output is None:
        args.output = f"models/mobilenet_torch_{args.weights}_detect_best.pt"
    ...
    model = build_model(num_classes, args.height, args.width, args.weights).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if map50 > best_map50:
            ...
            torch.save({
                ...
                "family": "mobilenet-retinanet-torch",
    ...
    emit(
        "\nNote: src/evaluate_models.py doesn't recognize the 'mobilenet-retinanet-torch' "
