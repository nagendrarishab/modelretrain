"""
    ...
    ./.venv/bin/python src/train/train_efficientnet_tf_detect.py --epochs 2 --batch-size 16

    # CPU run - pair with a GPU run of another backbone for real parallelism (see --device):
    ./.venv/bin/python src/train/train_efficientnet_tf_detect.py --epochs 2 --batch-size 4 --device cpu
"""
import argparse
...
def make_emitter(log_dir):
    ...
    log_path = Path(log_dir) / f"train_efficientnet_tf_{datetime.now():%Y%m%d_%H%M%S}.log"
