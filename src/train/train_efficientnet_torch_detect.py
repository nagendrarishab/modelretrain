"""
    python src/train/train_efficientnet_torch_detect.py --epochs 30
    ...
    ./.venv/bin/python src/train/train_efficientnet_torch_detect.py --epochs 2 --batch-size 16

    # CPU run - pair with a GPU run of another backbone for real parallelism (see --device):
    ./.venv/bin/python src/train/train_efficientnet_torch_detect.py --epochs 2 --batch-size 4 --device cpu
"""
import argparse
import os
...
def make_emitter(log_dir):
    ...
    log_path = Path(log_dir) / f"train_efficientnet_torch_{datetime.now():%Y%m%d_%H%M%S}.log"
...
class EfficientNetFPNBackbone(nn.Module):
    ...
    def __init__(self, efficientnet, height, width, out_channels=256):
        ...
        self.body = create_feature_extractor(
            efficientnet, return_nodes={
                "features.3": "p3",  # stride 8
                "features.5": "p4",  # stride 16
                "features.8": "p5",  # stride 32
            },
        )
        with torch.no_grad():
...
def build_model(num_classes, height, width):
    backbone = EfficientNetFPNBackbone(efficientnet_b0(weights=EfficientNet_B0_Weights.IMAGENET1K_V1), height, width)
    ...
    # for its 5-level default, just trimmed to the 3 levels EfficientNetFPNBackbone produces
...
def main():
    ...
    parser.add_argument("--output", default="models/efficientnet_torch_efficientnet_b0_detect_best.pt",
                         help="Output path")
    ...
    model = build_model(num_classes, args.height, args.width).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if map50 > best_map50:
            ...
            torch.save({
                ...
                "family": "efficientnet-retinanet-torch",
    ...
    emit(
        "\nNote: src/evaluate_models.py doesn't recognize the 'efficientnet-retinanet-torch' "
