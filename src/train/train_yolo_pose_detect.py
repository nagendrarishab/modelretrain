...
def main():
    ...
    log_entry = {
        ...
        "test_box_map50": float(metrics.box.map50),
        "test_box_map50_95": float(metrics.box.map),
        "test_pose_map50": float(metrics.pose.map50),
        "test_pose_map50_95": float(metrics.pose.map),
        "per_class_box_ap50": dict(zip(class_names, (float(x) for x in metrics.box.ap50))),
        "per_class_box_precision": dict(zip(class_names, (float(x) for x in metrics.box.p))),
        "per_class_box_recall": dict(zip(class_names, (float(x) for x in metrics.box.r))),
        "confusion_matrix": {
